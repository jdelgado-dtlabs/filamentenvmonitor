...
ensure_pysqlcipher3() {
    ...
    if ! "$INSTALL_ROOT/filamentcontrol/bin/python" -c "from pysqlcipher3 import dbapi2" 2>/dev/null && \
       ! "$INSTALL_ROOT/filamentcontrol/bin/python" -c "from sqlcipher3 import dbapi2" 2>/dev/null; then
        ...
        # For Python 3.13+, use sqlcipher3-wheels (pre-compiled, no C API issues)
        if "$INSTALL_ROOT/filamentcontrol/bin/python" -c "import sys; sys.exit(0 if sys.version_info >= (3, 13) else 1)" 2>/dev/null; then
            ...
            "$INSTALL_ROOT/filamentcontrol/bin/pip" install sqlcipher3-wheels==0.5.6
        else
...
generate_service_files() {
    echo -e "${CYAN}Generating systemd service file...${NC}"
    echo ""
    ...
    # Generate main service file (includes integrated Web UI)
...
EOF
    ...
    # Note: Web UI is now integrated into the main service via the orchestrator
    # No separate webui service needed

    if [ "$use_vault" = true ]; then
        echo -e "${GREEN}✓ Service file generated with HashiCorp Vault support${NC}"
        echo -e "${GREEN}  - filamentbox.service (includes integrated Web UI)${NC}"
        echo ""
        ...
        echo -e "${GREEN}✓ Service file generated (local key file mode)${NC}"
        echo -e "${GREEN}  - filamentbox.service (includes integrated Web UI)${NC}"
        echo ""
        echo -e "${CYAN}Configuration:${NC}"
        echo -e "${CYAN}  Installation path: $INSTALL_ROOT${NC}"
